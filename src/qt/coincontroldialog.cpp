...
{
    GUIUtil::setClipboard(ZuzcoinUnits::removeSpaces(contextMenuItem->text(COLUMN_AMOUNT)));
}
...
void CoinControlDialog::updateLabels(WalletModel *model, QDialog* dialog)
{
    ...
    int nDisplayUnit = ZuzcoinUnits::ZUZ;
    ...
    l2->setText(ZuzcoinUnits::formatWithUnit(nDisplayUnit, nAmount));        // Amount
    l3->setText(ZuzcoinUnits::formatWithUnit(nDisplayUnit, nPayFee));        // Fee
    l4->setText(ZuzcoinUnits::formatWithUnit(nDisplayUnit, nAfterFee));      // After Fee
    l5->setText(((nBytes > 0) ? ASYMP_UTF8 : "") + QString::number(nBytes));        // Bytes
    l7->setText(fDust ? tr("yes") : tr("no"));                               // Dust
    l8->setText(ZuzcoinUnits::formatWithUnit(nDisplayUnit, nChange));        // Change
    if (nPayFee > 0)
    {
    ...
    for (const std::pair<QString, std::vector<COutput>>& coins : mapCoins) {
        ...
        for (const COutput& out : coins.second) {
            ...
            if(ExtractDestination(out.tx->tx->vout[out.i].scriptPubKey, outputAddress))
            {
                ...
                // if listMode or change => show zuzcoin address. In tree mode, address is not shown again for direct wallet address outputs
            ...
            // amount
            itemOutput->setText(COLUMN_AMOUNT, ZuzcoinUnits::format(nDisplayUnit, out.tx->GetOutputValueOut(out.i)));
        ...
        if (treeMode)
        {
            ...
            itemWalletAddress->setText(COLUMN_AMOUNT, ZuzcoinUnits::format(nDisplayUnit, nSum));
