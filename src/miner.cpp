...
void static ZuzcoinMiner(const CChainParams& chainparams)
{
    CConnman& connman = *g_connman;
    LogPrintf("ZuzcoinMiner started\n");
    ...
    try
    {
        ...
        {
#ifndef HIM_NDEBUG
            std::cout << "HIM :: ZuzcoinMiner  coinbaseScript " << std::endl;
            if(coinbaseScript)
                std::cout << "HIM :: " << coinbaseScript->reserveScript.empty() << std::endl;
#endif
        ...
        while (true)
        {
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("Error in ZuzcoinMiner: Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;
            IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

            LogPrintf("Running ZuzcoinMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("ZuzcoinMiner terminated\n");
        throw;
    }
...
void GenerateZuzcoins(bool fGenerate, int nThreads, const CChainParams& chainparams)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ZuzcoinMiner, boost::cref(chainparams)));
}
