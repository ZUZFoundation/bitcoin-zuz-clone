...
UniValue getnewaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "\nReturns a new Zuzcoin address for receiving payments.\n"
            ...
            "\nResult:\n"
            "\"address\"    (string) The new zuzcoin address\n"
            "\nExamples:\n"
...
UniValue getaccountaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nDEPRECATED. Returns the current Zuzcoin address for receiving payments to this account.\n"
            "\nArguments:\n"
            "1. \"account\"       (string, required) The account name for the address. It can also be set to the empty string \"\" to represent the default account. The account does not need to exist, it will be created and a new address created  if there is no account by the given name.\n"
            "\nResult:\n"
            "\"address\"          (string) The account zuzcoin address\n"
            "\nExamples:\n"
...
UniValue getrawchangeaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
            ...
            "\nReturns a new Zuzcoin address, for receiving change.\n"
...
UniValue setaccount(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The zuzcoin address to be associated with an account.\n"
    ...
    if (!IsValidDestination(dest)) {
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Zuzcoin address");
    }
...
UniValue getaccount(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The zuzcoin address for account lookup.\n"
            "\nResult:\n"
...
UniValue getaddressesbyaccount(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "  \"address\"         (string) a zuzcoin address associated with the given account\n"
            "  ,...\n"
            "]\n"
            "\nExamples:\n"
...
static void SendMoney(CWallet * const pwallet, const CTxDestination &address, CAmount nValue, CAsset asset, 
    bool fSubtractFeeFromAmount, const CPubKey &confidentiality_key, CWalletTx& wtxNew, const CCoinControl& coin_control, bool fIgnoreBlindFail)
{
    CAmount curBalance = pwallet->GetBalance()[asset];
    ...
    if (!pwallet->CreateTransaction(vecSend, wtxNew, vChangeKey, nFeeRequired, nChangePosRet, strError, coin_control, NULL, true, NULL, true, NULL, NULL, NULL, fIgnoreBlindFail)) {
...
UniValue sendtoaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 10)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"            (string, required) The zuzcoin address to send to.\n"
            ...
            "                             The recipient will receive less zuzcoins than you enter in the amount field.\n"
    ...
    SendMoney(pwallet, dest, nAmount, asset, fSubtractFeeFromAmount, confidentiality_pubkey, wtx, coin_control, fIgnoreBlindFail);
...
UniValue listaddressgroupings(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  [\n"
            "    [\n"
            "      \"address\",            (string) The zuzcoin address\n"
...
UniValue signmessage(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 2)
        throw std::runtime_error(
            ...
            + HelpRequiringPassphrase(pwallet) + "\n"
            "\nArguments:\n"
            "1. \"address\"         (string, required) The zuzcoin address to use for the private key.\n"
...
UniValue getreceivedbyaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 3)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The zuzcoin address for transactions.\n"
...
UniValue getbalance(const JSONRPCRequest& request)
{
    ...
    if (account_value.isNull()) {
        ...
        return PushAssetBalance(pwallet->GetBalance(), pwallet, "");
    }
    ...
    if (request.params.size() > 3) {
        if (request.params[3].isStr()) {
            ...
            UniValue obj(UniValue::VOBJ);
            return PushAssetBalance(pwallet->GetBalance(), pwallet, assettype);
        }
    }
    ...
    if (request.params[0].get_str() == "*") {
        ...
        return PushAssetBalance(mapBalance, pwallet, "");
    }
...
UniValue getunconfirmedbalance(const JSONRPCRequest &request)
{
    ...
    CAmountMap balance = pwallet->GetUnconfirmedBalance();
    ...
    return PushAssetBalance(balance, pwallet, strasset);
...
UniValue sendfrom(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 3 || request.params.size() > 6)
        throw std::runtime_error(
            ...
            "\nDEPRECATED (use sendtoaddress). Sent an amount from an account to a zuzcoin address."
            + HelpRequiringPassphrase(pwallet) + "\n"
            "\nArguments:\n"
            ...
            "                       the spend.\n"
            "2. \"toaddress\"         (string, required) The zuzcoin address to send funds to.\n"
...
UniValue sendmany(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 10)
        throw std::runtime_error(
            ...
            "    {\n"
            "      \"address\":amount   (numeric or string) The zuzcoin address is the key, the numeric amount (can be string) in " + CURRENCY_UNIT + " is the value\n"
            "      ,...\n"
            "    }\n"
            ...
            "                           Those recipients will receive less zuzcoins than you enter in their corresponding amount field.\n"
            ...
            "       \"ECONOMICAL\"\n"
            "       \"CONSERVATIVE\"\n"
            "9. \"output_assets\"     (string, optional, default=bitcoin) a json object of assets to addresses\n"
            "   {\n"
            "       \"address\": \"hex\" \n"
            "       ...\n"
            "   }\n"
    ...
    for (const std::string& name_ : keys) {
    {
        ...
        if (!address.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid Zuzcoin address: ")+name_);
    ...
    for (unsigned int i = 0; i < setAssets.size(); i++) {
        vChangeKey.emplace_back(pwallet);
    }
    CAmount nFeeRequired = 0;
    int nChangePosRet = -1;
    string strFailReason;
    bool fCreated = pwallet->CreateTransaction(vecSend, wtx, vChangeKey, nFeeRequired, nChangePosRet, strFailReason,coin_control, NULL, true, NULL, false, NULL, NULL, NULL, fIgnoreBlindFail);
    if (!fCreated)
        throw JSONRPCError(RPC_WALLET_INSUFFICIENT_FUNDS, strFailReason);
    CValidationState state;
    if (!pwallet->CommitTransaction(wtx, vChangeKey, g_connman.get(), state)) {
...
UniValue addmultisigaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 4) {
        std::string msg = "addmultisigaddress nrequired [\"key\",...] ( \"account\" \"address_type\" )\n"
            ...
            "Each key is a Zuzcoin address or hex-encoded public key.\n"
            ...
            "2. \"keys\"                         (string, required) A json array of zuzcoin addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"address\"                  (string) zuzcoin address or hex-encoded public key\n"
            "       ...,\n"
            "     ]\n"
            ...
            "}\n"
            "\nResult (DEPRECATED. To see this result in v0.16 instead, please start zuzcoind with -deprecatedrpc=addmultisigaddress).\n"
            "        clients should transition to the new output api before upgrading to v0.17.\n"
            "\"address\"                         (string) A zuzcoin address associated with the keys.\n"
...
UniValue addwitnessaddress(const JSONRPCRequest& request)
{
    ...
    if (!IsDeprecatedRPCEnabled("addwitnessaddress")) {
        throw JSONRPCError(RPC_METHOD_DEPRECATED, "addwitnessaddress is deprecated and will be fully removed in v0.17. "
            "To use addwitnessaddress in v0.16, restart zuzcoind with -deprecatedrpc=addwitnessaddress.\n"
...
UniValue ListReceived(CWallet * const pwallet, const UniValue& params, bool fByAccounts)
{
    ...
    if (fByAccounts)
    {
        for (const auto& entry : mapAccountTally)
        {
            ...
            obj.push_back(Pair("amount",        PushAssetBalance(mapAmount, pwallet, strasset)));
...
UniValue listtransactions(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 4)
        throw std::runtime_error(
            ...
            "    \"address\":\"address\",    (string) The zuzcoin address of the transaction. Not present for \n"
...
UniValue listsinceblock(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 4)
        throw std::runtime_error(
            ...
            "    \"address\":\"address\",    (string) The zuzcoin address of the transaction. Not present for move transactions (category = move).\n"
...
UniValue gettransaction(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "      \"address\" : \"address\",          (string) The zuzcoin address involved in the transaction\n"
...
UniValue walletpassphrase(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 2) {
        throw std::runtime_error(
            ...
            "This is needed prior to performing transactions related to private keys such as sending zuzcoins\n"
            "\nArguments:\n"
...
UniValue encryptwallet(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1) {
        throw std::runtime_error(
            ...
            "\nNow set the passphrase to use the wallet, such as for signing or sending zuzcoin\n"
    ...
    StartShutdown();
    return "wallet encrypted; Zuzcoin server stopping, restart to run with encrypted wallet. The keypool has been flushed and a new HD seed was generated (if you are using HD). You need to make a new backup.";
...
UniValue lockunspent(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "A locked transaction output will not be chosen by automatic coin selection, when spending zuzcoins.\n"
...
UniValue listunspent(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 5)
        throw std::runtime_error(
            "listunspent ( minconf maxconf  [\"addresses\",...] [include_unsafe] [query_options] asset)\n"
            ...
            "3. \"addresses\"      (string) A json array of zuzcoin addresses to filter\n"
            "    [\n"
            ...
            "    }\n"
            "6.  \"asset\"                 (string, optional, default=all) The hex asset id or label to filter for.\n"
            "\nResult\n"
            "[                   (array of json object)\n"
            "  {\n"
            "    \"txid\" : \"txid\",          (string) the transaction id \n"
            "    \"vout\" : n,               (numeric) the vout value\n"
            "    \"address\" : \"address\",    (string) the zuzcoin address\n"
    ...
    if (!request.params[2].isNull()) {
        ...
        for (unsigned int idx = 0; idx < inputs.size(); idx++) {
            ...
            if (!IsValidDestination(dest)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Zuzcoin address: ") + input.get_str());
            }
    ...
    if (request.params.size() > 5 && request.params[5].isStr()) {
        assetstr = request.params[5].get_str();
    }
    CAsset asset;
    if (assetstr != "") {
...
UniValue fundrawtransaction(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 3)
        throw std::runtime_error(
                            ...
                            "   {\n"
                            "     \"changeAddress\"          (string, optional, default pool address) The zuzcoin address to receive the change\n"
    ...
    if (!request.params[1].isNull()) {
      if (request.params[1].type() == UniValue::VBOOL) {
        ...
        if (options.exists("changeAddress")) {
            ...
            if (!IsValidDestination(dest)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "changeAddress must be a valid zuzcoin address");
            }
...
//    }
    ...
    if (request.params.size() > 1) {
        ...
        if (options.exists("confTarget") && options.exists("totalFee")) {
            throw JSONRPCError(RPC_INVALID_PARAMETER, "confTarget and totalFee options should not both be set. Please provide either a confirmation target for fee estimation or an ex
    ...
    if (totalFee > 0) {
        ...
        if (totalFee < minTotalFee) {
            throw JSONRPCError(RPC_INVALID_PARAMETER, strprintf("Insufficient totalFee, must be at least %s (oldFee %s + incrementalFee %s)",
                                                                FormatMoney(minTotalFee), FormatMoney(nOldFeeRate.GetFee(maxNewTxSize)), FormatMoney(::incrementalRelayFee.GetFee(maxN
        }
        nNewFee = totalFee;
        nNewFeeRate = CFeeRate(totalFee, maxNewTxSize);
    } else {
...
//    CValidationState state;
//    if (!pwallet->CommitTransaction(wtxBumped, vChangeKey, g_connman.get(), state)) {
...
//        throw JSONRPCError(RPC_WALLET_ERROR, strprintf("Error: The transaction was rejected! Reason given: %s", state.GetRejectReason()));
    ...
    CFeeRate minMempoolFeeRate = mempool.GetMinFee(GetArg("-maxmempool", DEFAULT_MAX_MEMPOOL_SIZE) * 1000000);
    if (nNewFeeRate.GetFeePerK() < minMempoolFeeRate.GetFeePerK()) {
        throw JSONRPCError(RPC_MISC_ERROR, strprintf("New fee rate (%s) is less than the minimum fee rate (%s) to get into the mempool. totalFee value should to be at least %s or set
    }
...
{ //  category              name                        actor (function)           argNames
    ...
    { "wallet",             "dumpblindingkey",          &dumpblindingkey,          {} },
    { "wallet",             "dumpassetlabels",          &dumpassetlabels,          {} },
    { "wallet",             "dumpissuanceblindingkey",  &dumpissuanceblindingkey,  {"txid", "vin"} },
    { "wallet",             "claimpegin",               &claimpegin,               {"bitcoinT", "txoutproof", "claim_script"} },
    { "wallet",             "createrawpegin",           &createrawpegin,           {"bitcoinT", "txoutproof", "claim_script"} },
    { "wallet",             "getpeginaddress",          &getpeginaddress,          {} },
    { "wallet",             "importblindingkey",        &importblindingkey,        {} },
