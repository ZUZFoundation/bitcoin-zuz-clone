...
CAmount CScript::GetFraudBounty() const
{
    ...
    return CScriptNum(data, false, 8).getint();
}

bool CScript::IsWithdrawLock(const uint256 hashGenesisBlock, bool fRequireDestination, bool fRequireToUs) const //HIM_REVISIT_5
{
    ...
    if (fRequireToUs) {
        std::vector<unsigned char> vExpectedHash(20);
        //TODO: Require some fraud bounty
        CScript expectedScript = CScript() << OP_DROP << CScriptNum(144) << OP_LESSTHANOREQUAL;
        CHash160().Write(expectedScript.data(), expectedScript.size()).Finalize(vExpectedHash.data());
...
{
    assert(IsWithdrawLock(ArithToUint256(0)));

    const_iterator pc = begin();
    opcodetype opcode;
    std::vector<unsigned char> vchgenesishash;
...
static bool PopWithdrawPush(std::vector<std::vector<unsigned char> >& pushes, std::vector<unsigned char> *read=NULL) {
    if (pushes.empty())
        return false;
    ...
    for (int i = pushCount; i > 0; i--) {
        if (i != 1 && pushes[pushes.size() - i].size() != 520)
            return false;
        if (read != NULL) {
            const std::vector<unsigned char> &push = pushes[pushes.size() - i];
...
COutPoint CScript::GetWithdrawSpent() const
{
    ...
    try
    {
        ...
        while (pc < end())
        {
            pushes.push_back(std::vector<unsigned char>());
        ...
        CDataStream stream(vTx, SER_NETWORK, PROTOCOL_VERSION | SERIALIZE_VERSION_MASK_BITCOIN_TX);
...
void CScript::PushWithdraw(const std::vector<unsigned char> push) {
    int64_t pushCount = 0;
    for (std::vector<unsigned char>::const_iterator it = push.begin(); it < push.end(); ++pushCount) {
        if (push.end() - it < 520) {
            *this << std::vector<unsigned char>(it, push.end());
            it = push.end();
        } else {
            *this << std::vector<unsigned char>(it, it + 520);
            it += 520;
        }
    }
    *this << pushCount;
