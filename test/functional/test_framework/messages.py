...
"""Zuzcoin test framework primitive and message strcutures
...
# Objects that map to zuzcoind objects, which can be serialized/deserialized
...
class CTxOut(object):
    def __init__(self, nValue=CTxOutValue(), scriptPubKey=b'', nAsset=CTxOutAsset(ZUZCOIN_ASSET_OUT), nNonce=CTxOutNonce()):
        self.nAsset = nAsset
...
class CTransaction():
    ...
    def deserialize(self, f):
        ...
        if len(self.vin) == 0:
            ...
            # matches the implementation in zuzcoind
            if (flags != 0):
                self.vin = deser_vector(f, CTxIn)
...
class msg_headers():
    ...
    def deserialize(self, f):
        # comment in zuzcoind indicates these should be deserialized as blocks
