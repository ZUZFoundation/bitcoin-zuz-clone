#!/bin/bash
# Copyright (c) 2013 The Zuzcoin Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Zuzcoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Zuzcoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Zuzcoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Zuzcoin 0.7 datadir."
    ;;
  3)
    echo "Detected Zuzcoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected Zuzcoin 0.8 datadir."
    ;;
esac
