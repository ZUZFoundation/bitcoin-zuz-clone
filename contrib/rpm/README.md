...
The RPM spec file provided here is for Zuzcoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://zuzcoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Zuzcoin git repository](https://github.com/zuzcoin/zuzcoin) but are not part of
...
Currently only `Source10` is used, for the example `zuzcoin.conf` file.
...
in the zuzcoin git repository. At present only an SVG version of the Zuzcoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the zuzcoin
...
needed when 0.12.1 is released, a proper fix is already in the Zuzcoin git
master branch.
...
idea to build Zuzcoin Core against a static version of that library compiled
...
The `zuzcoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `zuzcoin-server` package. At this point, most RPM based
...
The `zuzcoin-server` package should have SELinux support. How to properly do
...
Zuzcoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/zuzcoin/). He has been
packaging Zuzcoin for Fedora at least since 2012.
...
4. I renamed the `zuzcoin` package that contains the Qt GUI to `zuzcoin-core` as
that appears to be how the general population refers to it, in contrast to
`zuzcoin-xt` or `zuzcoin-classic`. I wanted to make sure the general population
