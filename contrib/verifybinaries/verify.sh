...
###   zuzcoincore.org and zuzcoin.org and compares them.
...
WORKINGDIR="/tmp/zuzcoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://zuzcoincore.org"
HOST2="https://zuzcoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="zuzcoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'zuzcoin-',
...
if [ $? -ne 0 ]; then
   echo "zuzcoin.org failed to provide signature file, but zuzcoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "zuzcoin.org and zuzcoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Zuzcoin Core binary release signing key installed?"
   fi
